...
- **Cause** : `SeatRepository` exposait une And-chain à 4–5 propriétés ; le build APT Mansart **passait**
  mais l'impl générée **jetait au runtime**. L'exception remontait dans `RoomSocket.onText` →
  **avalée par le handler WebSocket Chappe** (pas de log) → diagnostic très difficile.
- **Contournement Arago (légitime, conservé)** : `SeatRepository` réduit à `findByRoomIdAndReleased`,
  filtrage pseudo/coordonnée en Java, l'index unique partiel `ux_seats_active` = autorité.
- **Corrigé upstream** :
  1. **Mansart `MANSART-002`** — `QueryMethodParser` supporte désormais les chaînes `And`/`Or` N-aires
     (split récursif avec backtracking) ; `RepositoryWriter` émet un **WARNING compile** (Messager) quand
     une méthode reste non dérivable au lieu d'un stub `UnsupportedOperationException` muet. Régression :
     `NaryDerivedQueryTest` ; reactor data 147 tests verts. (Le runner TCK Jakarta Data est cassé dans cet
     env — bootstrap Vauban enricher `null` — vérifié **préexistant** par comparaison baseline, indépendant.)
  2. **Chappe `CHAPPE-003`** — `WebSocketConnection.safeError` logge via `System.Logger` (WARNING pour une
     erreur handler, DEBUG pour le bruit I/O/protocole) avant de déléguer à `onError` (dont le défaut est
     un no-op). Régression : `WebSocketEchoTest.handlerErrorIsLoggedNotSwallowed` ; clean install chappe
     vert dont 53 tests de conformance RFC 6455.
